  ...
  return [...Array(props.outerCount)].map((_, i) => (
    <div key={i} ref={(d) => (groupRef.current[i] = d)}>
      {[...Array(props.innerCount)].map((_, j) => (
        ...
        >
          <p className={`${Math.random() > 0.5 ? "left" : "right"}-bubble-grey`}>&nbsp;</p>
        </div>
      ))}
    </div>
  ));
});
RandomBubbles.displayName = "Random Bubbles";

const outerCount = 10;
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const randomFactors = [...Array(outerCount)].map((_) => [
  Math.random().toFixed(1) * 1.1 + 0.6,
  Math.random().toFixed(2) * 1,
  Math.random().toFixed(1) * 10 + 2,
  Math.random().toFixed(1) * 10 + 2,
]);
          ...
          target={
            <Fragment>
              ...
              <div
                ...
                style={{
                  background: "linear-gradient(251deg, #21D4FD 0%, #B721FF 100%)",
                  top: "20vh",
                  left: "10vw",
                  boxShadow: "0 3px 3rem rgba(82, 80, 80, 0.405)",
                }}
              >
                <p className="left-bubble">
                  Shayan has demonstrated excellent communication skills, strong leadership, dilligence, compassion, and admirable helpfulness.
                </p>
                <div className="read-more" onClick={() => (window.location.href = "./LOR1.pdf")}>
                  Source
                </div>
              </div>
              <div
                className="recommendation"
                style={{ background: "linear-gradient(19deg, #21D4FD 0%, #B721FF 100%)", top: "25vh", marginLeft: "50vw" }}
              >
                <p className="right-bubble">
                  It is clear that he will be an exceptional asset to any institution or workforce he shall be a part of.
                </p>
                <div className="read-more" onClick={() => (window.location.href = "./LOR2.pdf")}>
                  Source
                </div>
              </div>
            </Fragment>

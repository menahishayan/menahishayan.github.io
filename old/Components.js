...
export const TechBadge = (props) => (
  <div className="tech-badge-container">
    <img
      ...
      alt={props.label}
      style={{ transform: `scale(${props.zoom || 1})`, cursor: props.url ? "pointer" : "default" }}
      onClick={() => {
        if (props.url) window.location.href = props.url;
      }}
    />
    <span className="tooltiptext" style={{ backgroundColor: props.color }}>
      {props.label}
    </span>
  </div>
);

export const techBadges = (props) =>
  props.map((t, i) => <TechBadge key={i} {...{ ..._techBadges[t.title ? t.title : t], label: t.title ? t.title : t, url: t.url || undefined }} />);

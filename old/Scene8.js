...
const rowLengths = techStack.map((t) => t.join(" ").length),
  minRL = Math.min(...rowLengths),
  rangeFactor = (Math.max(...rowLengths) - minRL) / 15;
const getHoverDistance = (j) => (j % 2 === 0 ? "-" : "") + (10 + (rowLengths[j] - minRL) / rangeFactor) + "vw";
const randoms = techStack.map((r) =>
  [...r.slice(-2), ...r, ...r.slice(0, 2)].map(() => ({ from: Math.random() / 6 + 0.05, to: Math.random() / 3 + 0.1 }))
);
        ...
        {props.list.map((r, j) => (
          <div key={j} style={{ whiteSpace: "nowrap" }} ref={(d) => (rows.current[j] = d)}>
            {[...r.slice(-2), ...r, ...r.slice(0, 2)].map((w, i) => (
              <span className="word-backdrop" key={i} ref={(s) => (words.current[j][i] = s)}>
                {w}
              </span>
            ))}
            <br />
...
HoveringWordsBackdrop.displayName = "HoveringWordsBackdrop";
